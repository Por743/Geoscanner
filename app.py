...
input_mode = st.sidebar.radio(
    "เลือกรูปแบบการกำหนดพื้นที่:", 
    ["กรอกพิกัด BBox Array", "อัปโหลดไฟล์ GeoJSON", "วาดขอบเขตบนแผนที่ (Draw)"]
)
...
# PREVIEW MAP
# ==========================================
st.subheader("📍 ตรวจสอบและกำหนดขอบเขตพื้นที่นำเข้า")

# กำหนดจุดศูนย์กลางแผนที่เริ่มต้น (ถ้ายังไม่มีข้อมูลให้ชี้ไปที่ภาคเหนือของไทย)
start_loc = [18.698561,98.846713]
zoom_start = 14
...
if st.session_state['current_roi_dict'] is not None:
    try:
        ...
        start_loc = [center[1], center[0]]
        zoom_start = 12
    except:
        pass

preview_map = folium.Map(location=start_loc, zoom_start=zoom_start)

# วาด Polygon เดิมลงบนแผนที่ (ถ้ามี)
if st.session_state['current_roi_dict'] is not None:
    folium.GeoJson(
...
# ---------------------------------------------------------
# เพิ่มเครื่องมือวาด (Draw Plugin) ถ้าผู้ใช้เลือกโหมด "วาดขอบเขตบนแผนที่"
# ---------------------------------------------------------
if input_mode == "วาดขอบเขตบนแผนที่ (Draw)":
    draw = Draw(
        export=False,
        position='topleft',
        draw_options={
            ...
            'marker': False,
            'circlemarker': False,
            'rectangle': True  # เปิดให้วาดเฉพาะกล่องสี่เหลี่ยม
        }
    )
    draw.add_to(preview_map)
    st.info("🖌️ คลิกที่ปุ่มรูปสี่เหลี่ยมด้านซ้ายของแผนที่ เพื่อลากวาด Bounding Box")

# รับค่าจากแผนที่ (ดึงเฉพาะข้อมูลการวาดรูปล่าสุดเพื่อลดความหน่วง)
map_data = st_folium(preview_map, width=1100, height=400, key="preview_map", returned_objects=["last_active_drawing"])

# หากมีการวาดรูปใหม่ ให้บันทึกทับลงใน Session State ทันที
if input_mode == "วาดขอบเขตบนแผนที่ (Draw)" and map_data.get("last_active_drawing"):
...
if run_button and st.session_state['current_roi_dict'] is not None:
    ...
    with st.spinner("⏳ กำลังดึงภาพถ่ายดาวเทียมและคำนวณแบบจำลองเชิงพื้นที่..."):
        try:
            ...
            # ---------------------------------------------------------
            # ตรวจสอบขนาดพื้นที่ (คำนวณเป็นตารางกิโลเมตร)
            # ---------------------------------------------------------
            area_sqkm = roi.area().getInfo() / 1e6
            max_area_sqkm = 500  # กำหนดลิมิตสูงสุด (เช่น 500 ตร.กม. คุณสามารถปรับตัวเลขนี้ได้ตามต้องการ)
            
            if area_sqkm > max_area_sqkm:
                st.error(f"⚠️ ขอบเขตพื้นที่มีขนาดใหญ่เกินไป ({area_sqkm:.2f} ตร.กม.)\nกรุณากำหนดพื้นที่ให้เล็กกว่า {max_area_sqkm} ตร.กม. เพื่อป้องกันการใช้หน่วยความจำเกินขีดจำกัด")
            else:
                st.success(f"ขนาดพื้นที่เป้าหมาย: {area_sqkm:.2f} ตารางกิโลเมตร (อยู่ในเกณฑ์ที่รองรับ)")
                
                # โค้ดส่วนดึงภาพและประมวลผลเดิมของคุณ ...
